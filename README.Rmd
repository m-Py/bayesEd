...
the use of the `BayesFactor` package [@R-BayesFactor].
...
an observed t-value.[^1] In the context of Bayesian statistics, hypotheses
...
[^1]: In general, Bayes factors do not require that one of the
hypotheses is a null hypothesis. But it is often the case, especially
...
$r$. The default scaling parameter is sqrt2/2 ≈ 0.71, which is the also
the default setting in the package `BayesFactor`.  The following plot
illustrates the scaling parameter:
...
scaling parameter sqrt2/2 ≈ 0.71. The following call illustrates the
predictions of the Cauchy prior and the null hypothesis for a sample
size of 100:
...
the data from the example above where I sampled 200 observations: 
